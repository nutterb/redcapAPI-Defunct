validateImport <- function(field, meta_data, records, ids,
  ...
  #* checkbox fields
    ...
    if (length(w) > 0){
      ...
      check_msg$msg <- paste0("Entry for '", field, "' must be either one of: 0, 1, Checked, Unchecked, ", checkChoice, ".")
  ...
  #* phone number fields
    ...
    if (length(w) > 0){
      ...
      phone_msg$msg <- paste0("Entry for '", field, "' must be a 10 digit phone number. This value was not uploaded.")
    ...
    if (length(w) > 0){
      phone_msg <- records[w, c(ids, field), drop=FALSE]
      phone_msg$msg <- paste0("Entry for '", field, "' is not a valid 10 digit phone number and is not imported.")
      printLog(phone_msg, logfile)
      x[w] <- NA
    }
  ...
  #* phone number fields
    ...
    if (length(w) > 0){
      ...
      email_msg$msg <- paste0("Entry for '", field,"' is not a valid e-mail address and is not imported.")
